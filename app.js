...
// let pElements = [
//   { symbol: "Na", aN: 11, name: "Sodium", aWeight: 22.99 },
//   { symbol: "Ga", aN: 31 },
//   { symbol: "O", aN: 8 },
//   { symbol: "F", aN: 9 },
//   { symbol: "N", aN: 7 },
...
let splitWord = (x = document.getElementById("convert-text").value) => {
  // if (!x) {
  //   x = document.getElementById("convert-text").value;
  // }
  let assembly = [];
  let key1 = "";
  let key2 = "";
  for (let i = 0; i < x.length; i++) {
    //Check 2 letters, than 1 letter
    if (x[i + 1]) {
      key2 = x[i] + x[i + 1];
      // console.log(key2);
      if (matchingSet(key2)) {
        assembly.push(matchingSet(key2));
        i += 2;
      }
    }
    ...
    if (key1) {
      if (matchingSet(key1)) {
        assembly.push(matchingSet(key1));
      } else {
        assembly.push(key1.toUpperCase());
      }
    }
  }
  ...
  for (let f = 0; f < assembly.length; f++) {
    let domElement = document.createElement("div");

    if (typeof assembly[f] == "object") {
      let elementAn = document.createElement("div");
      elementAn.innerText = assembly[f].atomicNumber;
      elementAn.className = "element-atomicnumber";
      domElement.appendChild(elementAn);

      let elementSymbol = document.createElement("div");
      elementSymbol.innerText = assembly[f].symbol;
      elementSymbol.className = "element-symbol";
      domElement.appendChild(elementSymbol);

      let elementName = document.createElement("div");
      elementName.innerText = assembly[f].name;
      elementName.className = "element-name";
      domElement.appendChild(elementName);

      let elementWeight = document.createElement("div");
      elementWeight.innerText = assembly[f].atomicMass;
      elementWeight.className = "element-weight";
      domElement.appendChild(elementWeight);

      domElement.className = "element-frame";
      // domElement.innerText = assembly[f].symbol;
    } else {
      domElement.className = "regular-text";
      domElement.innerText = assembly[f];
    }

    result.appendChild(domElement);
  }
...
//samples
//NaGa FUNNY GaBe PaGe RuTh LiTe RaCoON

//Have regular text in between elements (togglable)
let matchingSet = (key) => {
  for (let k = 0; k < pElements.length; k++) {
    if (Object.values(pElements[k])[17].toLowerCase() == key.toLowerCase()) {
      // console.log(
      //   "We found a match! " +
      //     "Element: " +
      //     Object.values(pElements[k])[17].toLowerCase() +
      //     " | Letter: " +
      //     key
      // );
      return pElements[k];
